 ...
 */
 const generateTwoWeekSummary = (incomes, expenses) => {
  ...
  const scuffedExpenses = expenses.map((expense) =>  ({value: -expense.value, date: new Date(expense.date)}))
  const scuffedIncomes = incomes.map((income) =>  ({value: income.value, date: new Date(income.date)}))

  const transactions = scuffedExpenses.concat(scuffedIncomes)
